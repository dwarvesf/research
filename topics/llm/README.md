---
title: § LLM
description: Explore comprehensive guides on AI foundations, prompt engineering, model training, retrieval, evaluation, AI agents, security, and real-world applications of large language models for effective AI development.
date: 2025-04-23
authors:
  - tieubao
tags:
  - llm
  - moc
redirect:
  - /s/nJowBQ
---

### Foundations

- [A grand unified theory of the AI hype cycle](a-grand-unified-theory-of-the-ai-hype-cycle.md)
- [Foundation model](foundation-model.md)
- [Model context protocol](model-context-protocol.md)
- [Model selection](model-selection.md)
- [Quantization in LLM](quantization-in-llm.md)

### Prompt engineering

- [Easy Prompt engineering For Business Use And Mitigating Risks In LLM](easy-prompt-engineering-for-business-use-and-mitigating-risks-in-llms.md)
- [How to talk to ChatGPT effectively](how-to-talk-to-chatgpt-effectively.md)

### Model training & fine-tuning

- [Exploring machine learning approaches for fine tuning Llama models](exploring-machine-learning-approaches-for-fine-tuning-llama-models.md)
- [RLHF with Open Assistant](rlhf-with-open-assistant.md)
- [Reinforcement learning](reinforcement-learning.md)
- [Reward model](reward-model.md)
- [Proximal policy optimization](proximal-policy-optimization.md)
- [Q learning](q-learning.md)

### Retrieval & caching

- [Caching with RAG system](caching-with-rag-system.md)
- [Chunking strategies to overcome context limitation in LLM](chunking-strategies-to-overcome-context-limitation-in-llm.md)
- [Dealing with long-term memory in AI chatbot](dealing-with-long-term-memory-in-ai-chatbot.md)
- [Hybrid search](hybrid-search.md)
- [Raptor LLM retrieval](raptor-llm-retrieval.md)
- [Re-ranking in RAG](re-ranking-in-rag.md)
- [LLM query caching](llm-query-caching.md)
- [Select vector database for LLM](select-vector-database-for-llm.md)
- [Multimodal in RAG](multimodal-in-rag.md)
- [Workaround with OpenAI's token limit with Langchain](workaround-with-openai-s-token-limit-with-langchain.md)
- [Working with Langchain document loaders](working-with-langchain-document-loaders.md)

### Evaluation & metrics

- [Evaluation guideline for LLM application](evaluation-guideline-for-llm-application.md)
- [LLM as a judge](llm-as-a-judge.md)
- [Feedback mechanism](feedback-mechanism.md)
- [Logs pillar](logs-pillar.md)
- [Metric pillar](metric-pillar.md)
- [Observability in AI platforms](observability-in-ai-platforms.md)
- [Trace pillar](trace-pillar.md)
- [Thumbs up and thumbs down pattern](thumbs-up-and-thumbs-down-pattern.md)

### AI agents & workflows

- [Building agent supervisors to generate insights](supervisor-ai-agents.md)
- [Multi-agent collaboration for task completion](multi-agent-collaboration-for-task-completion.md)
- [ReAct (Reason + Act) in LLM](react-in-llm.md)
- [ReWOO in LLM](rewoo-in-llm.md)
- [Function calling](function-calling.md)
- [Supervisor AI agents](supervisor-ai-agents.md)

### Security & guardrails

- [Guardrails in LLM](guardrails-in-llm.md)
- [Prevent prompt injection](prevent-prompt-injection.md)

### Use cases & applications

- [Build your chatbot with open source large language models](build-your-chatbot-with-open-source-large-language-models.md)
- [Building LLM powered tools with Dify](building-llm-powered-tools-with-dify.md)
- [Generative UI](generative-ui.md)
- [LLM tracing in AI system](llm-tracing-in-ai-system.md)
- [The rise of AI applications with LLM](the-rise-of-ai-applications-with-llm.md)
- [Use cases for LLM applications](use-cases-for-llm-applications.md)
- [Text to MongoDB](text-to-mongodb.md)
